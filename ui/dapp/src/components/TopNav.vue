<template>
  <nav class="navbar navbar-expand-lg py-0 topnav d-flex flex-row fixed-top"
       v-bind="$attrs"
       :style="{background:bg ? bg : ''}">
    <router-link :to="{name: 'FileBrowser'}" class="navbar-brand">
      <svg data-v-8ba516f6="" width="135" viewBox="0 0 187 55" xmlns="http://www.w3.org/2000/svg"><title
        data-v-8ba516f6="" color="#fff">logo</title>
        <g data-v-8ba516f6="" fill="none" fill-rule="evenodd">
          <path data-v-8ba516f6=""
    ...
    <div class="topnav-buttons ml-auto">
      <div class="ml-auto flex-row d-flex align-items-center">
        <div v-if="connectionStatus === 'offline'"
             class="d-inline-block">
          <i class="mdi md-24 mdi-cloud-off-outline text-muted"
             v-tooltip="$t('filebrowser.connectionStatus.offline', 'You are offline')"></i>
        </div>
        <div v-if="connectionStatus === 'syncing' || connectionStatus === 'starting'"
             class="d-inline-block mx-1">
          <i class="mdi md-24 mdi-sync text-muted mdi-flip-v"
             v-tooltip="$t('filebrowser.connectionStatus.syncing', 'Syncing with Blockchain')"></i>
        </div>
        <button class="btn btn-link allowance"
                data-tour-step="2"
                @click="$emit('walletModalOpened')">{{ $t('filebrowser.allowance.label', 'Allowance:') }} <span
          class="allowance--num">{{ allowance }} </span> <span class="allowance--currency">XES</span>
        </button>
        <button class="btn btn-light px-2 btn-notifications"
                @click="$emit('notificationModalOpened')"
                :class="{ disabled: unreadNotificationsCount === 0 && pendingNotificationsCount === 0}"
                data-tour-step="6">
          <span class="mdi md-24 mdi-bell"></span>
          <span class="badge badge-info badge-num"
                v-if="pendingNotificationsCount">{{ pendingNotificationsCount }}</span>
        </button>
        <button slot="buttons" type="button" class="btn btn-primary ml-1"
                data-tour-step="1"
                @click="$emit('walletModalOpened')">{{ $t('filebrowser.buttons.wallet', 'Wallet') }}
        </button>
        <button class="btn btn-primary ml-2" @click="toggledShowAddressBook" data-tour-step="3">
          {{ $t('filebrowser.buttons.addressbook', 'Address Book') }}
        </button>

        <b-dropdown id="export-dropdown" text="Dropdown Button" variant="link" ref="userProfileDropDown">
          <template slot="button-content">
            <i class="mdi mdi-account md-24"></i>
          </template>
          <!--<button class="dropdown-item" data-tour-step="7" @click.prevent="">-->
          <!--<language-select/>-->
          <!--</button>-->
          <b-dropdown-item @click="tour">{{ $t('proxeus.tour.taketour', 'Proxeus tour') }}</b-dropdown-item>
          <b-dropdown-item @click="lockAccount">{{ $t('filebrowser.buttons.logout', 'Logout') }}</b-dropdown-item>
        </b-dropdown>
      </div>
    </div>
  </nav>
...
export default {
  ...
  props: [
    'sm', 'bg', 'showAddressBook'
  ],
  computed: {
    allowance () {
      return this.$store.state.wallet.allowance
    },
    // TODO: replace with some reasonable impl
    connectionStatus () {
      return this.$store.state.wallet.connectionStatus
    },
    ...
    userProfileDropDown () {
      return this.$store.state.file.userProfileDropDown
    }
  },
  watch: {
    userProfileDropDown: function (dd) {
      let ref = this.$refs.userProfileDropDown
      if (dd === true) {
        ref.showMenu()
        ref.visible = true
      } else {
  ...
  methods: {
    ...
    exportInfo () {
      this.$emit('exportedInfo')
    },
    toggledShowAddressBook () {
      this.$emit('toggledShowAddressBook')
    },
    tour () {
...
<style lang="scss" scoped>
  ...
  // do not use mdi-spin class from materialdesignicons as the spinner would spin in the wrong direction
  @keyframes spin {
    0% {
  ...
  .nav-link {
    ...
    &.active {
      font-weight: 500;
      border-bottom: 4px solid theme-color("info");
    }
  }
