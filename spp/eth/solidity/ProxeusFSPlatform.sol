...
contract ProxeusFS {
    ...
    function createFileUndefinedSigners(bytes32 hash, bytes32 data, uint mandatorySigners, uint expiry, bytes32 replacesFile, address[] prvs, uint xesAmount) public {
        //address[] memory test = new address[](-1);
        _createFile(2, hash, data, new address[](mandatorySigners), expiry, replacesFile, prvs, xesAmount);
    }

    function createFileShared(bytes32 hash, bytes32 data, uint mandatorySigners, uint expiry, bytes32 replacesFile, address[] prvs, address[] readers, uint xesAmount) public {
        //address[] memory test = new address[](-1);
        _createFile(2, hash, data, new address[](mandatorySigners), expiry, replacesFile, prvs, xesAmount);
        fileSetPerm(hash,readers);
    }

    function createFileDefinedSigners(bytes32 hash, bytes32 data, address[] definedSigners, uint expiry, bytes32 replacesFile, address[] prvs, uint xesAmount) public {
        _createFile(3, hash, data, definedSigners, expiry, replacesFile, prvs, xesAmount);
    }

    function _createFile(uint fileType, bytes32 hash, bytes32 data, address[] definedSigners, uint expiry, bytes32 replacesFile, address[] prvs, uint xesAmount) internal {
        ...
        if(lfileType == 0){
            ...
            require(ERC20IF(XES_ADDRESS).allowance(msg.sender, this) >= 10**17);
            require(ERC20IF(XES_ADDRESS).transferFrom(msg.sender, this, 10**17));
            require(ERC20IF(XES_ADDRESS).transfer(owner, 10**17));
            ...
            for (uint ii = 0; ii < prvs.length; ii++){
                ...
                //todo: add function-parameter for xesAmount
                //todo: check if it is safe to emit here and what data exactly has to be sent
            ...
            eternalstorage.setFileData(hash,data);
